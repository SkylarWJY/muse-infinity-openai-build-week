...
async function curateJourney() {
  ...
  try {
    ...
    narrate([{
      speakerId: "mira",
      text: "Your route is ready. The company will keep refining it while you enter the first world."
    }], { replace: true });
...
function showConversationTurns(items, completeLabel, { allowSkip = true } = {}) {
  ...
  const result = view.showCompanionConversation(turns, { onCompleteLabel: completeLabel, allowSkip });
...
function recordStationEvidence({ choiceValue = "", observation = "" } = {}) {
  try {
    ...
    if (!usedLivePerspectives) {
      showConversationTurns(perspectives, sceneTour.metrics.resolved >= sceneTour.requiredStationCount
        ? "Reflect on this world"
        : "Continue to the next artwork", { allowSkip: false });
    }
    syncQuestionProgress();
...
async function enterFinalWorld() {
  ...
  try {
    ...
    view.toast("09 / ANSWER · your company has returned to you");
    state.dialogueGeneration += 1;
    state.dialogueBusy = false;
    state.currentSceneId = scene.id;
    view.setSpeaker(MIRA_SPEAKER);
    ...
    narrate([{
      speakerId: "mira",
      text: `You carried “${journey.question}” through eight worlds, and your company has returned with you. ${state.salon?.principle || journey.manifesto} This journey is complete; the answer remains yours to live with.`,
