...
export class MuseumEngine {
  constructor(container, { onGuideState = () => {}, onMetrics = () => {}, onFollowChange = () => {}, onWorldLayerStatus = () => {} } = {}) {
    ...
    this.player = new ProceduralAvatar({ coat: 0xe7e2d7, accent: 0xff6b4a, skin: 0x704737, scale: 0.93, name: "Learner" });
  ...
  async init() {
    const world = await this.setWorld("gallery");
    this.ready = true;
    this.animate();
    return world;
  }
  ...
  async setWorld(worldId) {
    ...
    if (this.activeStopId) this.worldLayer.highlight(this.activeStopId);
    return world;
  }

  showSalonCharacters(visible = true) {
    if (!visible) {
      for (const actor of this.salonActors) this.scene.remove(actor.group);
      this.salonActors = [];
      return;
    }
    if (this.salonActors.length) return;
    const definitions = [
      { name: "The Maker", coat: 0x315b59, accent: 0x63dfe0, x: -3.1, z: -3.8 },
      { name: "The Witness", coat: 0x6b352e, accent: 0xff6b4a, x: 0, z: -5.2 },
      { name: "The Skeptic", coat: 0x4b5034, accent: 0xd8ff42, x: 3.1, z: -3.8 }
    ];
    for (const item of definitions) {
      const actor = new ProceduralAvatar({ coat: item.coat, accent: item.accent, skin: 0x875a48, scale: 0.92, name: item.name });
      actor.group.position.set(item.x, 0, item.z);
      actor.group.lookAt(0, 0, 1.5);
      actor.setMotion(0, "open");
      this.scene.add(actor.group);
      this.salonActors.push(actor);
    }
  }

  navigateTo(stopId) {
    const stop = getSceneStop(stopId);
  ...
  updatePlayer(dt) {
    ...
    if (manual) {
      ...
      const behind = new THREE.Vector3(0, 0, 1.7).applyAxisAngle(new THREE.Vector3(0, 1, 0), this.guide.group.rotation.y);
    ...
    this.player.group.position.x = THREE.MathUtils.clamp(this.player.group.position.x, -7.35, 7.35);
    this.player.group.position.z = THREE.MathUtils.clamp(this.player.group.position.z, -9.8, 7.2);
  ...
  updateCamera(dt) {
    ...
    const offset = new THREE.Vector3(
      ...
      2.5 + Math.sin(-this.cameraPitch) * this.cameraDistance,
  ...
  animate = () => {
    ...
    this.guide.update(dt, this.elapsed);
    for (const actor of this.salonActors) actor.update(dt, this.elapsed);
